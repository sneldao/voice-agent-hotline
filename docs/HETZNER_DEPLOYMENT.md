...
App directory: `/opt/voice-hotline-celo` — Port 3042 — PM2: `voice-hotline-celo`
...
`scripts/deploy.sh` does the following automatically:
...
rm -rf node_modules out    → drop build-only deps
write .env.hetzner         → PM2 reads this for secrets
pm2 delete + start ecosystem.config.js  → reload with new config
pm2 save                   → persist PM2 state
```
...
| `CELO_RPC_URL` | `https://forno.celo.org` | Optional override |
...
3. `make deploy` — deploy.sh writes the new credentials to `.env.hetzner`
4. PM2 picks up the new values on restart