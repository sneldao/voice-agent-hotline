...
export function DeskInstrument({ stage, label = 'PAPER TRADING / NO LIVE ORDERS' }: { stage: DeskInstrumentStage; label?: string }) {
  ...
  return (
    <div ref={hostRef} className={styles.instrument} data-ready={ready} data-stage={stage} aria-hidden="true">
      <div className={styles.instrumentFallback}>
        <div className={styles.fallbackReceiver}><i /><i /></div>
        <div className={styles.fallbackBody}>
          <span className={styles.fallbackDisplay}>CLAFLIN<br /><strong>HETTY · PRIVATE LINE</strong><small>{label}</small></span>
          <span className={styles.fallbackDial} />
          <span className={styles.fallbackEdge} />
        </div>
      </div>
      <canvas ref={canvasRef} className={styles.instrumentCanvas} />
    </div>
  );
}
