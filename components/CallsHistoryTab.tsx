      ...
      <div className="flex gap-2">
        ...
        {displayCalls.length > 0 && (
          <button
            ...
            className="px-4 py-2 rounded-lg text-sm font-medium bg-black/25 text-amber-100/50 hover:bg-amber-100/10 transition-colors"
          >
            Export
          </button>
        )}
...
function StatCard({ icon, value, label }: { icon: React.ReactNode; value: string; label: string }) {
  return (
    <div className="rounded-xl border border-amber-100/15 bg-[#17100d]/85 p-3 text-center">
      <div className="text-amber-100/50 mb-1">{icon}</div>
      <div className="text-lg font-bold text-amber-50">{value}</div>
      <div className="text-xs text-amber-100/45">{label}</div>
    </div>
  );
}
  ...
  return (
    <button
      ...
      className={`flex-1 py-2 px-4 rounded-lg text-sm font-medium transition-colors ${
        active ? 'bg-red-500/20 text-amber-50 border border-red-400/50' : 'bg-black/25 text-amber-100/50 hover:bg-amber-100/10'
      }`}
    >
      {children}
    </button>
      ...
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center justify-between">
            ...
            <button onClick={onToggleSave} className={`p-1 rounded ${isSaved ? 'text-red-400' : 'text-amber-100/25 hover:text-amber-100/50'}`}>
              <Bookmark className={`w-4 h-4 ${isSaved ? 'fill-current' : ''}`} />
            </button>
          </div>
          <div className="text-sm text-amber-100/50 mb-2">{call.agentSpecialty}</div>
          <div className="flex items-center gap-4 text-xs text-amber-100/45">
            <span className="flex items-center gap-1">
              <Clock className="w-3 h-3" />{formatDuration(call.duration)}
            </span>
            <span>{formatDate(call.timestamp)}</span>
            <span className="text-red-400">${(call.cost || 0).toFixed(2)}</span>
          </div>
          <div className="flex items-center gap-1 mt-2">
            {Array.from({ length: 5 }).map((_, i) => (
              ...
              >
                <Star className={`w-3.5 h-3.5 ${i < (hoverRating || displayRating) ? 'text-yellow-400 fill-yellow-400' : 'text-amber-100/25'}`} />
              </button>
            ))}
            {!isRated && <span className="text-xs text-amber-100/25 ml-1">Tap to rate</span>}
          </div>
        </div>
      </div>
      <div className="flex gap-2 mt-3 pt-3 border-t border-amber-100/15">
        <button onClick={onViewTranscript} className="flex-1 py-2 text-xs font-medium text-amber-100/50 hover:text-amber-50 bg-black/25 rounded-lg hover:bg-amber-100/10">
          Transcript
        </button>
        <button onClick={() => setShowActions(!showActions)} className="px-3 py-2 text-xs font-medium text-amber-100/50 hover:text-amber-50 bg-black/25 rounded-lg hover:bg-amber-100/10">
          {showActions ? 'Less' : 'More'}
        </button>
      </div>
      {showActions && (
        <div className="flex gap-2 mt-3 pt-3 border-t border-amber-100/15">
          <button onClick={onDownload} className="flex-1 py-2 text-xs font-medium text-amber-100/50 hover:text-amber-50 bg-black/25 rounded-lg hover:bg-amber-100/10">
            Download
          </button>
          <button onClick={onShare} className="flex-1 py-2 text-xs font-medium text-amber-100/50 hover:text-amber-50 bg-black/25 rounded-lg hover:bg-amber-100/10">
            Share
          </button>
          <button onClick={onCallAgain} className="flex-1 py-2 text-xs font-medium text-amber-100/50 hover:text-amber-50 bg-black/25 rounded-lg hover:bg-amber-100/10">
            Call Again
          </button>
        </div>
      )}
